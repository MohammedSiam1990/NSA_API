...
namespace POS.Common
{
    public class Base36
    {
        ...
        static private string CharList36
        {
            get
            {
                if (_CharList36.Length < 62)
                {
                    char[] array = new char[62];
                    for (int i = 0; i < 10; i++) array[i] = (char)(i + 48);
                    for (int i = 0; i < 26; i++) array[i + 10] = (char)(i + 65);
                    for (int i = 0; i < 26; i++) array[i + 36] = (char)(i + 97);
        ...
        private static ulong Decode36(string input)
        {
            ...
            {
                result += (ulong)CharList36.IndexOf(c) * (ulong)Math.Pow(62, pos);
                pos++;
            }
            return result;
        }
